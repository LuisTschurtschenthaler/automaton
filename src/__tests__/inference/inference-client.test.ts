...
describe("UnifiedInferenceClient", () => {
  ...
  it("chat uses survival tier resolution when credits are low", async () => {
    ...
    // Survival mode downgrades reasoning → fast; preferred fast provider is groq,
    // but anthropic now has a fast-tier model and comes before groq in fallback.
    // The first available fast provider wins.
    expect(result.metadata.tier).toBe("reasoning");
  });
